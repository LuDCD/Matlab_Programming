...
for v = vmin:t:vmax
    ...
    for i = 1:5
     ...
     %{
       h1 = 1*Alpha(1) + 1*Alpha(2) + 1*Alpha(3) + 1*Alpha(4) + 1*Alpha(5);
       H = 18 - h1;
       a = F(5)*cos(Alpha(5))/dm(n);
       L = a*tan(0.0017)+  sqrt( (H-a)^2 - a*a );
       %}
     L = -(tan(Alpha(5))-tan(0.0017))/dm(n)*F(5)*cos(Alpha(5));
     X1 = -( log(sec(Theta(5))+tan(Theta(5)))-log(sec(0.0017)+tan(0.0017)) )/dm(n)*F(5)*cos(Alpha(5));
     % 0.0017rad = 0.1度
...
% vt = vt*0.714;
% plot(X,vt);
% title( '游动区域直径与风速的关系'); %书写图名
% xlabel( '风速v' );	% 	s为字符串
% ylabel( '浮动区域直径X(m)' );
% axis( [0,35,0,30] );%设定坐标的范围，可以取inf，-inf
% grid on; %显示分格线
% hold on;
